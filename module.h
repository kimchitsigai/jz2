...
struct TORCH_API Module {
  ...
  void register_buffer(const std::string& name, autograd::Variable v) {
    std::cout << "Module::register_buffer() : " << name << "\n";
  ...
  void register_parameter(
      ...
      autograd::Variable v,
      bool is_buffer) {
    std::cout << "Module::register_parameter() : " << name << "\n";
    set_or_add_slot(
        name,
        TensorType::get(),
        v,
  ...
  void register_module(const std::string& name, const Module& module) {
    std::cout << "Module::register_module() : " << name << "\n";
    set_or_add_slot(
 ...
 private:
  ...
  void set_or_add_slot(
      ...
      IValue v,
      EntityType etype) {
    if (strcmp(name.c_str(),"weight")==0 || strcmp(name.c_str(),"bias"))
      std::cout << "c10 set_or_add_slot : " <<  name << " , slot_type = " << slot_type << "\n";
    else
      std::cout << "c10 set_or_add_slot : " <<  name << " , slot_type = " << slot_type << "\n";
    ...
    if(find_slot(name, etype))
      std::cout << "c10 set_or_add_slot : found slot after operation" << "\n";
    else
      std::cout << "c10 set_or_add_slot : !!! found NO slot after operation !!!" << "\n";
  }
  ...
  c10::optional<Slot> find_slot(const std::string& name, EntityType etype)
    ...
    if (!slot) {
      if(etype == EntityType::MODULE)
        std::cout << "c10 find_slot name=" << name << "  etype=MODULE" << ": slot de findAttributeSlot() = NULL " << "\n";
      else if(etype == EntityType::PARAMETER) 
        std::cout << "c10 find_slot name=" << name << "  etype=PARAMETER" << ": slot de findAttributeSlot() = NULL " << "\n";
      else if(etype == EntityType::ATTRIBUTE) 
        std::cout << "c10 find_slot name=" << name << "  etype=ATTRIBUTE" << ": slot de findAttributeSlot() = NULL " << "\n";
      else if(etype == EntityType::METHOD) 
        std::cout << "c10 find_slot name=" << name << "  etype=METHOD" << ": slot de findAttributeSlot() = NULL " << "\n";
      else 
        std::cout << "c10 find_slot name=" << name << "  etype=UNKNOWN" << ": slot de findAttributeSlot() = NULL " << "\n";
      return c10::nullopt;
    }
    Slot r = get_slot(*slot);
    if (r.entity_type() != etype) {
      std::cout << "c10 find_slot name=" << name << "  etype != r.entity_type()" << "\n";
      return c10::nullopt;
    }
    return r;
  }
