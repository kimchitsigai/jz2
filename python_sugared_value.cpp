...
std::shared_ptr<SugaredValue> ModuleValue::attr(
  ...
  std::cout << "ModuleValue::attr() in python_sugared_value.cpp START(), field = " << field << "\n"; 

  if (field == "training") {
    c10::optional<Slot> v = module_.find_attribute(field);
    if (!v) {
      bool training = py::cast<bool>(py::getattr(py_module_, "training"));
      std::cout << "ModuleValue::attr() in python_sugared_value.cpp register_attribute() \n"; 
  ...
  if (auto v = module_.find_module(field)) {
    std::cout << "ModuleValue::attr() in python_sugared_value.cpp find_module(field) not null returning() \n"; 
    ...
    std::cout << "ModuleValue::attr() in python_sugared_value.cpp all first class returning() \n"; 
  ...
  std::cout << "ModuleValue::attr() in python_sugared_value.cpp after py::getattr\n";
  ...
  if (py::isinstance<py::function>(attr) &&
    ...
    for (auto name : param_names) {
      std::cout << "ModuleValue::attr() in python_sugared_value.cpp before g.insertGetAttr()loop \n";
      params.emplace_back(g.insertGetAttr(self_, py::str(name)));
    }
    auto list = g.insertNode(g.createTuple(params))->output();
    std::cout << "ModuleValue::attr() in python_sugared_value.cpp returning after g.insertNode()\n";
  ...
  if (py::isinstance(attr, py::module::import("torch.nn").attr("Module"))) {
    std::cout << "ModuleValue::attr() in python_sugared_value.cpp after attr isinstance import(torch.nn).attr(Module)\n";
    ...
    std::cout << "ModuleValue::attr() in python_sugared_value.cpp after py::module::import(torch::jit).attr() \n";
    if (!result.is_none()) {
      ...
      std::cout << "ModuleValue::attr() in python_sugared_value.cpp before register_module()\n"; 
      module_.register_module(field, *submodule);
      std::cout << "ModuleValue::attr() in python_sugared_value.cpp before find_module()\n";
      auto v = module_.find_module(field);
      std::cout << "ModuleValue::attr() in python_sugared_value.cpp before ModuleValue::insertGetAttr()and returning ModuleValue\n";
      return std::make_shared<ModuleValue>(
          m.graph()->insertGetAttr(self_, field), *v, result);
    }
  } else if (py::isinstance<py::function>(attr)) {
    std::cout << "ModuleValue::attr() in python_sugared_value.cpp is_instance<function> py::jit::import(torch::jit)\n";
  ...
  std::cout << "ModuleValue::attr() in python_sugared_value.cpp did you forget to add it __constants__? \n";
...
std::vector<std::shared_ptr<SugaredValue>> ModuleValue::asTuple(
  ...
  for (Slot s : module_.get_module_slots()) {
    std::cout << "ModuleValue::asTuple() module_.get_module_slots" << "\n";
