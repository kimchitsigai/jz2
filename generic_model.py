...
class GenericModel(torch.jit.ScriptModule):
    ...
    def __init__(self, game_params: GameParams, model_params: ModelParams):
        # Modif Jin
        print("GenericModel.__init() START\n")
        #super(GenericModel, self).__init__()
        #print("GenericModel.__init() after super.init()\n")
        torch.jit.ScriptModule.__init__(self)
        print("GenericModel.__init() after jit.ScriptModule\n")
        ...
        print("GenericModel._init() END \n")

    # Modif Jin
    @torch.jit.script_method
    def _forward(self, x: torch.Tensor, return_logit: bool):
        print("GenericModel.____forward() START\n")
        ...
        print("GenericModel.____forward() END\n")
        return v, pi

    @torch.jit.script_method
    def forward(self, x: torch.Tensor):
        print("GenericModel.forward() START\n")
        v, pi = self._forward(x, False)
        pi = pi.view(-1, self.c_prime, self.h_prime, self.w_prime)
        reply = {"v": v, "pi": pi}
        print("GenericModel.forward() END\n")
        return reply
