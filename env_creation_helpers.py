...
def create_model(
    ...
    if model_params.model_name is not None:
        ...
        #x = torch.randn(128, c, h, w)
        #model = torch.jit.trace(model_zoo.GenericModel(game_params=game_params, model_params=model_params), x)
        #model = torch.jit.script(model_zoo.GenericModel(
        #    game_params=game_params, model_params=model_params
        #))
    ...
    for child_name, child_model in model.named_children():
        print(f"                      child_name: {child_name}, child_model: {child_model}", child_name, child_model);
    return model
