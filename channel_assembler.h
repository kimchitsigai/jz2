...
class ChannelAssembler {
 public:
  ChannelAssembler(int actBatchsize,
    ...
    fmt::printf("ChannelAssembler() numActChannel = %d, jitModel = %s\n", numActChannel, jitModel);
...
#ifdef PYTORCH12
      fmt::printf("ChannelAssembler() LOADING model i = %d, device = %s, jitModel = %s\n", i, device, jitModel_);
      models_.push_back(std::make_shared<TorchJitModel>(torch::jit::load(jitModel_)));
      // models_ = vecteur de TorchJitModel = torch::jit::script::Module déclaré à la fin de ce fichier
      fmt::printf("ChannelAssembler() after models_.push_back(std::make_shared<TorchJitModel>(torch::jit::load(jitModel_)))\n");
#else
      fmt::printf("NOT PYTORCH 12 LOADING model ChannelAssembler() i = %d, device = %s\n", i, device);
  ...
  void loadModelStateDict(
      TorchJitModel& model,
      const std::unordered_map<std::string, torch::Tensor>& stateDict) {
    fmt::printf("++++++++++++++++++++++++++++ loadModelStateDict() ++++++++++++++++++++++++++++\n");

    int status = 0;
    char * demangled = abi::__cxa_demangle(typeid(model).name(),0,0,&status);
    std::cout << "type de model = " << demangled << "\n";

    //for (auto submod : model.get_modules()) {
      // Declare iterator to a slot_list
      /*** OK
      torch::jit::script::slot_list_impl<torch::jit::script::Module>::iterator ptr = model.get_modules().begin();
      std::cout << "(*slot) de submod de model.get_modules() = " << (*ptr).is_optimized() << "\n";
      ***/
    //torch::jit::script::slot_list_impl<torch::jit::script::Module>::iterator ptr = model.get_modules().begin();

    torch::jit::script::slot_list_impl<torch::jit::script::Module> lst_mod = model.get_modules();
    torch::jit::script::slot_list_impl<torch::jit::script::Module>::iterator it_mod = lst_mod.begin() ;
    for( ;it_mod!=lst_mod.end(); ++it_mod) {
        std::cout << "       get_modules().iterator -> module.name : " << (*it_mod).name().qualifiedName() << "\n";
    }
    torch::jit::script::slot_list_impl<torch::jit::script::Slot> lst_slot = model.get_parameters();
    torch::jit::script::slot_list_impl<torch::jit::script::Slot>::iterator it_slot = lst_slot.begin() ;
    for( ;it_slot!=lst_slot.end(); ++it_slot) {
        std::cout << "       get_parameters().iterator -> slot(parameter).name : " << (*it_slot).name() << "\n";
    }

    for (auto& [name, tensor] : stateDict) {
      const char* ptr = name.c_str();
      // Modif Jin
      // torch::jit::script::Module* currentModule = &model;
      ...
      while (*ptr) {
        if (*ptr == '.') {
          ...
          // Modif Jin
          // torch::jit::script::Module subModule = (torch::jit::script::Module) (currentModule->find_module(memberNameString));
          // fmt::printf("subModule cast & found\n");
          ...
          if (!subModule) {
            ...
            fmt::printf("       copyModelStateDict: Known state dict entry '%s' -- module '%s'\n", name, memberNameString);
            torch::jit::script::Module submod = subModule.value();
            torch::jit::script::slot_list_impl<torch::jit::script::Module> lst_mod = submod.get_modules();
            torch::jit::script::slot_list_impl<torch::jit::script::Module>::iterator it_mod = lst_mod.begin() ;
            for( ;it_mod!=lst_mod.end(); ++it_mod) {
                std::cout << "                  get_modules().iterator -> (sub)module.name : " << (*it_mod).name().qualifiedName() << "\n";
            }
            torch::jit::script::slot_list_impl<torch::jit::script::Slot> lst_slot = submod.get_parameters();
            torch::jit::script::slot_list_impl<torch::jit::script::Slot>::iterator it_slot = lst_slot.begin() ;
            for( ;it_slot!=lst_slot.end(); ++it_slot) {
                std::cout << "                  get_parameters().iterator -> (sub)slot(parameter).name : " << (*it_slot).name() << "\n";
            }
            /***
            for (auto submod : subModule.get_modules()) {
              std::cout << "submod de subModule.get_modules() = " << submod << "\n";
            }
            for (auto subparam : subModule.get_parameters()) {
              std::cout << "subparam de subModule.get_parameters() = " << subparam << "\n";
            }
            ***/
          }
          currentModule = &*subModule;

          demangled = abi::__cxa_demangle(typeid(currentModule).name(),0,0,&status);
          std::cout << "type de subModule = " << demangled << "\n";
          free(demangled);

          fmt::printf("copyModelStateDict: submodule '%s'\n", currentModule);
          ++ptr;
          memberNamePtr = ptr;
        } else {
          ++ptr;
      ...
      memberNameString.assign(memberNamePtr, ptr - memberNamePtr);
      fmt::printf("copyModelStateDict: memberNamePtr '%s'\n", memberNamePtr);
      fmt::printf("copyModelStateDict: ptr '%s'\n", ptr);
      fmt::printf("copyModelStateDict: memberNameString '%s'\n", memberNameString);

#ifdef PYTORCH12
      fmt::printf("copyModelStateDict: #ifdef PYTORCH12\n");
      if (c10::optional<torch::jit::script::Slot> p = currentModule->find_parameter(memberNameString); p) {
        torch::jit::script::Slot sl = p.value();
        sl.value().toTensor().copy_(tensor);
      } else if (c10::optional<torch::jit::script::Slot> b = currentModule->find_buffer(memberNameString); b) {
        torch::jit::script::Slot sl = b.value();
        sl.value().toTensor().copy_(tensor);
      } else {
#else
      fmt::printf("copyModelStateDict: NOT #ifdef PYTORCH12\n");
  ...
  void updateModel(
      const std::unordered_map<std::string, torch::Tensor>& stateDict) {
    torch::NoGradGuard ng;
    fmt::printf(" -- UPDATE MODEL -- models_.size=%d\n", models_.size());
    if (server_) {
