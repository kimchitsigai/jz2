...
void Module::save(const std::string& filename, const ExtraFilesMap& extra_files)
    const {
  std::cout << "Module::save() filename = " << filename << "\n" ;
#ifndef C10_MOBILE
...
void Module::copy_into(
  ...
  for (Slot s : curr.get_slots()) {
    if (s.is_module()) {
      std::cout << "Module::copy_into() push_back : " << s.name() << "\n" ;
      ...
      std::cout << "Module::copy_into() curr.set_or_add_slot de : " << s.name() << "\n" ;
...
void Module::clone_method(const Module& orig, const std::string& name) {
  ...
  std::cout << "Module::clone_method() start" << "\n";
  while (!to_scan.empty()) {
  ...
  std::cout << "Module::clone_method() end" << "\n";
...
slot_list Module::get_parameters() const {
  std::cout << "Module::get_parameters() \n" ;
